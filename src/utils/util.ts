import { isArray, isString, isStringNumber } from "@/utils/is"
 ...
 * @return array
 */
export function filterCurrentRoute(menuList: any, path: string) {
  let result = {}
...
/**根据modules获取路由数组 */
export function getRoutesByModules(
  _modules: any,
  systemKey?: string
  ...
  Object.keys(_modules).forEach((key) => {
    ...
    result.push(
      ...moduleList.map((item) => {
        if (systemKey) item.system = [systemKey]
        return item
      })
    )
  })
